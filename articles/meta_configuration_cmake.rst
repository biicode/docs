...
The user source code is arranged in blocks, each one in the form *username/blockname*. These blocks are the units that are published and reused, somewhat (but different, really) similar to repositories in a version control system.
...
In this way, it is very simple for the user to define or customize the building process, as they can edit the used variables in the CMakeLists.txt between the two included .cmake files.
Note that this approach is not exclusive to the possibility that the user has they own CMakeLists inside their blocks, together with their source code, or even other .cmake files that could be included from the biicode generated CMakeList.txt one.
...
For example, imagine that the user **willy** has develop a similar application, that says goodbye instead of hello, with the following layout::
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^
Suppose that you need to use a library, let's say Boost. If such library was already in biicode, it would be enough to just #include it. But boost is still not in biicode (and it is not likely to be in the short term, for many reasons, as being very large or having many complicated inter-dependencies between its libraries). Fortunately, as explained above, it is fairly straightforward to modify the CMakeLists.txt in order to account for it.

For example, if the required library is lambda, which is only composed by headers, it is only necessary to specify your boost installation directory as:
...
Of course, you can also use FIND_PACKAGE features of CMake for such purpose. In fact, we are already using it to find and configure projects with dependencies to well known, large and massively used libraries as WxWidgets or Boost, so it will be enough for users to just #include what they want and biicode is able to fully configure the project if a local installation of such libraries is found.
